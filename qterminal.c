...
void read_responce() {
  ...
  do {
    //  usleep(waittime*100); // задержка ожидания ответа - не нужна, termios
    //  сам ее отрабатывает
    rlen = read(siofd, ibuf + dlen, 5000); // ответ команды
    if ((dlen + rlen) >= 5000)
      break; // переполнение буфера
    dlen += rlen;
  } while (rlen != 0);
  ...
  if (hexflag) {
    ...
    if (wrapperlen == 0)
      ...
      for (i = 0; i < dlen; i += wrapperlen) {
        if ((dlen - i) < wrapperlen)
          clen = dlen - i; // длина последней строки
