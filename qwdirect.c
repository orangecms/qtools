...
void set_linux_format() {
  ...
  if (nand_ecc_cfg != 0xffff)
    cfgecctemp = mempeek(nand_ecc_cfg);
  else
    cfgecctemp = 0;
  sparnum =
      6 -
      ((((cfgecctemp >> 4) & 3) ? (((cfgecctemp >> 4) & 3) + 1) * 4 : 4) >> 1);
  // Для ECC- R-S
  if (!(is_chipset("MDM9x25") || is_chipset("MDM9x3x") ||
        is_chipset("MDM9x4x")))
    set_blocksize(516, 1, 10); // data - 516, spare - 1 байт, ecc - 10
  // Для ECC - BCH
  else {
...
\n");
  ...
  if ((wmode == w_standart) || (wmode == w_linux))
    oobsize = 0; // для входных файлов без OOB
  ...
  //-------------------------------------------
  if (cflag) {
    if ((startblock + cflag) > maxblock)
      cflag = maxblock - startblock;
    printf("\n");
  ...
  if (wmode == w_linout)
    bsize = pagesize * ppb; // для этого режима файл не содержит данных OOB, но
                            // требуется запись в OOB
  else
    bsize = (pagesize + oobsize * spp) *
            ppb; // размер в байтах полного блока флешки, с учетом ООВ
  ...
  if ((i % bsize) != 0)
    fsize++; // округляем вверх до границы блока
  ...
  for (block = startblock; block < (startblock + flen); block++) {
    ...
    for (page = 0; page < ppb; page++) {
      ...
      memset(srcbuf, 0xff,
             pagesize); // заполняем буфер FF для чтения неполных страниц
      // читаем весь дамп страницы
      if (wmode == w_linout)
        readlen = fread(srcbuf, 1, pagesize, in);
      else
        readlen = fread(srcbuf, 1, pagesize + (spp * oobsize), in);
      if (readlen == 0)
        goto endpage; // 0 - все данные из файла прочитаны
      ...
      if (test_badpattern(srcbuf)) {
        ...
        if (!usflag) {
          if (page == 0)
            printf("\n Обнаружен признак дефектного блока во входном дампе - "
                   "пропускаем\n");
        ...
        if (ucflag) {
          ...
          mark_bad(block);
          if (page == 0)
            printf("\r Блок %x отмечен как дефектный в соответствии с входным "
                   "файлом!\n",
                   block);
          continue;
        }
        ...
        if (umflag && badflag && page == 0)
          printf("\r Блок %x - дефекты соответствуют, продолжаем запись\n",
                 block);
      } else if (umflag && badflag) {
        printf("\n Блок %x: на flash обнаружен неожиданный дефект, завершаем "
               "работу!\n",
               block);
        return;
      }
      ...
      case w_linux:
      case w_image:
        // для всех режимов, кроме yaffs и linout - формат входного файла
        // 512+obb
        for (i = 0; i < spp; i++) {
          memcpy(databuf + sectorsize * i, srcbuf + (sectorsize + oobsize) * i,
                 sectorsize);
          if (oobsize != 0)
            memcpy(oobuf + oobsize * i,
                   srcbuf + (sectorsize + oobsize) * i + sectorsize, oobsize);
        }
        break;
      ...
      printf("\r Блок: %04x   Страница: %02x", block, page);
      fflush(stdout);
      setaddr(block, page);
      ...
      case w_linout:
        mempoke(nand_cmd,
                0x39); // запись data+spare, ECC вычисляется контроллером
        break;

      case w_image:
        mempoke(nand_cmd, 0x39); // запись data+spare+ecc, все данные из буфера
                                 // идут прямо на флешку
        break;
      }
      ...
      for (sector = 0; sector < spp; sector++) {
        memset(datacmd, 0xff,
               sectorsize +
                   64); // заполняем секторный буфер FF - значения по умолчанию

        // заполняем секторный буфер данными
        switch (wmode) {
        case w_linux:
          // линуксовый (китайский извратный) вариант раскладки данных, запись
          // без OOB
          if (sector < (spp - 1))
            //первые n секторов
            memcpy(datacmd, databuf + sector * (sectorsize + 4),
                   sectorsize + 4);
          else
            // последний сектор
            memcpy(
                datacmd, databuf + (spp - 1) * (sectorsize + 4),
                sectorsize -
                    4 * (spp - 1)); // данные последнего сектора - укорачиваем
          break;
        ...
        case w_image:
          ...
          memcpy(datacmd + sectorsize, oobuf + sector * oobsize,
                 oobsize); // oob
          break;
        ...
        case w_yaffs:
          ...
          // входной файл имеет формат page+oob, но при этом тег лежит с позиции
          // 0 OOB
          if (sector < (spp - 1))
            //первые n секторов
            memcpy(datacmd, databuf + sector * (sectorsize + 4),
            ...
            memcpy(
                ...
                sectorsize -
                    4 * (spp - 1)); // данные последнего сектора - укорачиваем
            memcpy(datacmd + sectorsize - 4 * (spp - 1), oobuf,
                   16); // тег yaffs присоединяем к нему
          }
          break;
      ...
      if (!vflag)
        continue; // верификация не требуется
                  // верификация данных
      printf("\r");
      setaddr(block, page);
          ...
          if (sector != (spp - 1)) {
            ...
            for (i = 0; i < sectorsize + 4; i++)
              if (membuf[i] != databuf[sector * (sectorsize + 4) + i])
                printf("! block: %04x  page:%02x  sector:%u  byte: %03x  %02x "
                       "!= %02x\n",
                       block, page, sector, i, membuf[i],
                       databuf[sector * (sectorsize + 4) + i]);
          } else {
            // последний сектор
