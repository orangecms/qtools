...
void defect_list(int start, int len) {
  ...
  for (blk = start; blk < (start + len); blk++) {
    printf("\r Проверка блока %08x", blk);
    fflush(stdout);
    if (check_block(blk)) {
      printf(" - badblock");
      fprintf(out, "\n%08x", blk);
    ...
    for (pg = 0; pg < ppb; pg++) {
      ...
      for (sec = 0; sec < spp; sec++) {
        ...
        if ((stat == -1) && (flag == 1))
          continue;
        if (stat == -1) {
          printf("\r!  Блок %x  Страница %d  сектор %d: некорректируемая "
                 "ошибка чтения\n",
                 blk, pg, sec);
          fprintf(out,
                  "\r!  Блок %x  Страница %d  сектор %d: некорректируемая "
                  "ошибка чтения\n",
                  blk, pg, sec);
        } else {
          printf(
              "\r!  Блок %x  Страница %d  сектор %d: скорректировано бит: %d\n",
              blk, pg, sec, stat);
          fprintf(
              out,
              "\r!  Блок %x  Страница %d  сектор %d: скорректировано бит: %d\n",
