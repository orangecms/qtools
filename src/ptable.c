...
int load_ptable_flash() {
  ...
  for (blk = 0; blk < 12; blk++) {
    ...
    if (memcmp(buf, "\xac\x9f\x56\xfe\x7a\x12\x7f\xcd", 8) != 0)
      continue; // сигнатура не найдена - ищем дальше
    ...
    if ((fptable.magic1 != FLASH_PART_MAGIC1) ||
        (fptable.magic2 != FLASH_PART_MAGIC2))
      continue;
    // нашли таблицу
    validpart = 1;
    // корректируем длину последнего раздела
    if ((maxblock != 0) &&
        (fptable.part[fptable.numparts - 1].len == 0xffffffff))
      fptable.part[fptable.numparts - 1].len =
          maxblock - fptable.part[fptable.numparts - 1]
                         .offset; // если длина - FFFF, то есть растущий раздел
...
int show_part(int pn) {
  ...
  if (!validpart)
    return 0; // таблица еще не загружена
  if (pn >= fptable.numparts)
    return 0; // неправильный номер раздела
  ...
  printf("%02x %02x %02x %02x   %s   %.16s\n", fptable.part[pn].attr1,
         fptable.part[pn].attr2, fptable.part[pn].attr3,
         fptable.part[pn].which_flash,
         (fptable.part[pn].attr2 == 1) ? "LNX" : "STD", fptable.part[pn].name);
...
void list_ptable() {
  ...
  if (!validpart)
    return; // таблица еще не загружена
  print_ptable_head();
...
//*************************************
char *part_name(int pn) { return fptable.part[pn].name; }

//*********************************************
// Получение стартового блока по номеру раздела
//*********************************************
int part_start(int pn) { return fptable.part[pn].offset; }

//*********************************************
// Получение длины раздела по номеру раздела
//*********************************************
int part_len(int pn) { return fptable.part[pn].len; }
