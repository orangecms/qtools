...
//* Получение errno
//****************************************************
int efs_get_errno() { return efs_errno; }

//****************************************************
//* Установка альтернативного флга
//****************************************************
void set_altflag(int val) { efs_altflag = val; }
...
int efs_stat(char *filename, struct efs_filestat *fi) {
  ...
  if (iolen == -1)
    return -1; // ошибка приема ответа
  efs_errno = fi->diag_errno;
  if (efs_errno != 0)
    return 0; // файл не найден или другая какая ошибка
  if (S_ISDIR(fi->mode))
    return 2;
  return 1;
}
...
int efs_readdir(int dirp, int seq, struct efs_dirent *rsp) {
  ...
  if (iolen == -1)
    return -1;
  efs_errno = rsp->diag_errno;
  // printf("\n rsp: dirp=%i seq=%i entry=%i mode=%08x
  // name=%s",rsp->dirp,rsp->seqno,rsp->entry_type,rsp->mode,rsp->name);
  // fflush(stdout);
  return efs_errno;
}
...
//******************************************************
int efs_factimage_read(int state, int sent, int map, int data,
