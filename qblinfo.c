...
void main(int argc, char *argv[]) {
  ...
  for (i = 0; i < fl; i++) {
    if (hwidfound == 0) {
      if (((memcmp(buf + i, hwidstr, 6)) == 0) &&
          (buf[i - 17] == 0x30)) { // ищем строку "HW_ID" в сертификате
    ...
    if ((i >= 0x1000) && (i % 4 == 0) &&
        (tablefound ==
         0)) { // таблица выровнена по границе слова и ещё не обработана
      daddr = *((unsigned int *)&buf[i]); // адрес заглушки
      if ((daddr < 0x10000) || (daddr > 0xffff0000))
        continue;
      if ((daddr != *((unsigned int *)&buf[i - 4])) &&
        ...
        for (j = 0; j < 0x100; j++) {
          if (*((unsigned int *)&buf[i - 4 + 4 * j]) != daddr) {
            if ((*((unsigned int *)&buf[i - 4 + 4 * j]) & 0xfffc0000) !=
                (daddr & 0xfffc0000))
              break; // это не адрес, конец таблицы
            ncmd++;
            printf("\n CMD %02x = %08x", j + 1,
                   *((unsigned int *)&buf[i - 4 + 4 * j]));
          }
        }
        if (ncmd != 0) {
